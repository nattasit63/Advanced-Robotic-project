...
class VRPPD():
    ...
    def open_config(self):
        ...
        my_path_default = "$HOME/interface_ws/src/ui/ui/"
        # my_path_default = "/home/natta/adv_ws/src/interface/config"
        root_filename = filedialog.askopenfilename(initialdir=my_path_default,title='Select .json file',filetypes=(("json file","*.json"),("all files","*.*")))
    ...
    def plt_show(self,path):
        ...
        for route in path:
            ...
            for k in range(len(route)-1):
                ...
                x = int((float(self.nodepos[route[k]][0])*10))
                y = int((float(self.nodepos[route[k]][1])*10))
                new_x = int((float(self.nodepos[route[k+1]][0])*10))
                new_y = int((float(self.nodepos[route[k+1]][1])*10))
